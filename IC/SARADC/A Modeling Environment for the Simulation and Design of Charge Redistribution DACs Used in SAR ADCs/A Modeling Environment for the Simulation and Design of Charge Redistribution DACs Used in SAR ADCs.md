...
1. （可能是公式错误）![ ](images/image.png) 对于DNL的标准差，DNL跳变最大的时候在MSB跳变(2^(N-1)个小电容),同时由于大电容是小电容并联，所以标准差不是简单叠加，而是要开根号，所以公式应当为$$\sigma_{\text{DNL},CBW} = 2^{\frac{N-1}{2}} \cdot \frac{\sigma_C}{C_u}$$
其他结构的电容阵列DNL和INL同理都要-1
德累斯顿的论文的公式就是-1的
2. 论文里输出电压的公式简化了，少考虑了顶板电容和电荷注入![ ](images/image-1.png)
